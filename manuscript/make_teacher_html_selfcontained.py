...
html_path = Path("paper2_draft_for_teacher_selfcontained.html")

md = md_path.read_text(encoding="utf-8")

# Convert formula-like bare LaTeX lines into code blocks to avoid rendering issues
lines = md.splitlines()
new_lines = []
...
# Embed local images as base64 so the HTML can be opened alone
def embed_image(match):
    ...
    # If relative path does not exist from current directory, try figures folder
    if not image_path.exists():
        image_path = Path("figures") / Path(src).name
...
body = re.sub(r'<img alt="([^"]*)" src="([^"]*)"\s*/?>', embed_image, body)
...
print("Created paper2_draft_for_teacher_selfcontained.html")
